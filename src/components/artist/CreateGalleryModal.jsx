...
const CreateGalleryModal = ({ isOpen, onClose, onSuccess }) => {
  ...
  const validateForm = () => {
    ...
    if (!formData.name.trim()) {
      ...
      newValidation.name = { 
        valid: false, 
        message: `Name too short (${nameLength}/3 characters minimum)`
      };
    } else if (nameLength > 50) {
      newValidation.name = { 
        valid: false, 
        message: `Name too long (${nameLength}/50 characters maximum)`
      };
    }

    // Enhanced description validation with character count
    if (!formData.description.trim()) {
      ...
      newValidation.description = { 
        valid: false, 
        message: `Description too short (${descriptionLength}/10 characters minimum)`
      };
    } else if (descriptionLength > 500) {
      newValidation.description = { 
        valid: false, 
        message: `Description too long (${descriptionLength}/500 characters maximum)`
      };
    }
    ...
    // Enhanced price validation with proper currency formatting
    if (formData.basePrice !== '') {
      const price = parseFloat(formData.basePrice);
      if (isNaN(price) || price < 0) {
        newValidation.basePrice = { 
          valid: false, 
        ...
        newValidation.basePrice = { 
          valid: false,
          message: `Price cannot exceed ${new Intl.NumberFormat('en-US', {
            style: 'currency',
  ...
  return !isOpen ? null : (
    <div className="fixed inset-0 overflow-y-auto bg-black/40 backdrop-blur-md z-50">
      <div className="min-h-screen px-4 py-8 text-center">
        ...
        <div className={`inline-block w-full max-w-4xl h-[calc(100vh-4rem)] text-left align-middle transition-all transform ${isDarkMode ? 'bg-gray-800/95' : 'bg-white/95'} backdrop-blur-xl rounded-2xl shadow-2xl relative overflow-hidden`}>
          {/* Glass effect border */}
          ...
          <button
            onClick={onClose}
            className={`absolute top-2 right-2 p-2 rounded-xl z-50 transition-all duration-300 transform hover:scale-110 ${
              isDarkMode 
          ...
          <div className="h-full flex flex-col">
            <div className="p-8 pb-4">
              ...
              {error && (
                <div className={`p-4 rounded-xl border-2 backdrop-blur-sm flex items-center gap-3 animate-fadeIn mb-6 ${
                  isDarkMode 
            ...
            <form onSubmit={handleSubmit} className="flex-1 flex flex-col overflow-hidden">
                    ...
                    <div className="max-h-[50vh] overflow-y-auto p-1">
                      {loadingImages ? (
                        ...
                        <div className="text-center py-12">
                          {searchTerm ? (
                            <>
                              <Search className="w-12 h-12 text-gray-400 mx-auto mb-4 animate-bounce" />
                            ...
                            <>
                              <Plus className="w-12 h-12 text-gray-400 mx-auto mb-4 animate-pulse" />
                            ...
                            return (
                              ...
                              >
                                <div className="aspect-square relative bg-gray-100 dark:bg-gray-800">
                                  ...
                                  {/* Selection status */}
                                  <div className={`absolute top-2 right-2 w-6 h-6 rounded-full flex items-center justify-center transition-all duration-300 transform ${
                                    isSelected 
                                      ? 'bg-blue-500 text-white scale-100' 
                                      : 'bg-white/80 text-gray-600 scale-0 group-hover:scale-100'
                                  }`}>
                                    {isSelected ? (
              ...
              <div className="flex gap-4 p-8 border-t border-gray-200 dark:border-gray-700 bg-inherit">
                <button
                  type="submit"
                  disabled={loading || (hasFormBeenTouched && !Object.values(formValidation).every(v => v.valid))}
                  className={`flex-1 px-8 py-3 relative overflow-hidden rounded-xl font-semibold transition-all duration-300 ${
                    loading || (hasFormBeenTouched && !Object.values(formValidation).every(v => v.valid))
                      ? 'opacity-50 cursor-not-allowed bg-gray-400 dark:bg-gray-600' 
                      : 'bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 transform hover:scale-105 text-white'
                  }`}
                  aria-busy={loading}
                >
                  <span className={`flex items-center justify-center gap-2 transition-all duration-300 ${
                    ...
                    <span>
                      Create Gallery 
                      {formData.imageIds.length > 0 && ` (${formData.imageIds.length} image${formData.imageIds.length !== 1 ? 's' : ''})`}
                    </span>
                  </span>
                  {loading && (
                    <span className="absolute inset-0 flex items-center justify-center gap-2">
                      <div className="animate-spin rounded-full h-4 w-4 border-2 border-white border-t-transparent"></div>
                      <span>Creating Gallery... {loadingProgress}%</span>
                    </span>
                  )}
                </button>
